...
(function () {
  ...
  const CONFIG = {
    ...
    mobileNavLinkSelector: '.mobile-nav-link',
  ...
  function initNavigation() {
    ...
    const mobileNavLinks = document.querySelectorAll(CONFIG.mobileNavLinkSelector);
        ...
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            ...
            const activeMobileLink = document.querySelector(
              `${CONFIG.mobileNavLinkSelector}[href="#${targetId}"]`
            );
            ...
            if (activeLink) {
              ...
              mobileNavLinks.forEach((link) => {
                link.setAttribute(
                  'aria-pressed',
                  link === activeMobileLink ? 'true' : 'false'
                );
              });
            }
          }
    ...
    // Scroll suave ao clicar nos links mobile
    mobileNavLinks.forEach((link) => {
      link.addEventListener('click', (e) => {
        const href = link.getAttribute('href');
        if (href && href.startsWith('#')) {
  ...
  function initMobileMenu() {
    ...
    // Fechar menu ao clicar fora dele (comportamento padrão)
